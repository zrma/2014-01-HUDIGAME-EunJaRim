...
/*
 * file: common/web-global.js
 *
 * Global functions for web
 */

function initToggleLoginPopup() {
    // Opening/Closing login popup
    
    jQuery('.web .login-toggle').click(function(e){
        e.preventDefault();

        jQuery(this).parents('.user-profile').toggleClass('closed open');
        jQuery('.user-profile.open .login-popup').find('input#mail').focus();

    });
}
function initSubmenuPolish() {
    /* Fixing IE7 menu problems */
    jQuery('.web.ie7 .#main_menu .with-submenu').mouseenter(function(){
        if(!jQuery(this).hasClass('measured')){
            var newWidth = jQuery(this).find('.submenu').css('width');
            jQuery(this).find('.submenu-list').children().width(newWidth);
            jQuery(this).addClass('measured');
        }
    });
}
function initButtonFocus() {
    
    /* Adding focus effect to custom buttons */
    jQuery('.web .button:not(.nostyle)').append('<span class="button-focus"></span>');
    
    /* Showing/Hiding focus for Input */
    jQuery('.web .button:not(.disabled) button,.web .button:not(.nostyle) button').focus(function(){
        $(this).parent('.button').addClass('focus');
    });
    jQuery('.web .button:not(.disabled) button,.web .button:not(.nostyle) button').blur(function(){
        $(this).parent('.button').removeClass('focus');
    });
    
    /* Showing/Hiding focus for A */
    jQuery('.web a.button:not(.disabled),.web a.button:not(.nostyle)').focus(function(){
        //$(this).addClass('focus');
    });
    jQuery('.web a.button:not(.disabled),.web a.button:not(.nostyle)').blur(function(){
        //$(this).removeClass('focus');
    });
}
jQuery(document).ready(function($) {
    initToggleLoginPopup();    
    initSubmenuPolish();    
    initButtonFocus();    
});

/*
 * file: common/soldierlist.js
 * author: Anders Adlén
 *
 * Interacting with soldier list on the web
 */
(function($){
$(document).ready(function($){
	$('.web .pick input[type=radio]:checked').prev('.soldier').addClass('selected');
	$('.web .pick .soldier').click(function(e){				
		if(!($(this).parents().hasClass('disabled'))){
			$(this).siblings('.soldier').removeClass('selected');
			$(this).parent('div').siblings('div').find('.soldier').removeClass('selected');
			$(this).addClass('selected').next('input[type=radio]').not(':disabled').attr("checked", "checked");
		}else{
			e.preventDefault();
		}
    });
...
/*
 * file: common/loading.js
 *
 * Adding a loading icon to buttons with class="loader"
 */
...
function resetLoadingSpinner() {
    jQuery('.web .button-view.inprogress').removeClass('inprogress focus').bind('click');
    jQuery('.button.loader').removeClass('inprogress').find('.loading').hide().remove();
    jQuery('.button.loader .button-content *').removeClass('invisible');
    
    jQuery('.web.stripped span.continue, .web .continue-btn span.continue').hide().remove();
}

function initStrippedLoadingSpinner() {
    /*jQuery('.web.stripped .button, .web .button.continue-btn').append('<span class="continue" style="display:block;"></span>');
    jQuery('.web.stripped span.continue, .web .continue-btn span.continue').hide();*/
    var isLoading = 0;
    jQuery('.web.stripped .button-view, .web .button-view.button-view-login, .web .button-view.continue-btn').not('.disabled').not(':disabled').click(function(){
        var $this = jQuery(this);
        
        if(isLoading == 0){
            $this.addClass('inprogress loading disabled');
            isLoading++
        }
        
        // Disable clicked button to prevent double submitting
        $this.unbind('click');
        setTimeout(function () {
...
function resetStrippedLoadingSpinner() {
    jQuery('.web.stripped .button').delay(2000).removeClass('inprogress').find('.continue').hide().remove();
}

jQuery(window).unload(function($) {
    jQuery('.web.stripped span.continue, .web .continue-btn span.continue').remove();
});

jQuery(document).ready(function($) {
    initLoadingSpinner();
    
    if(jQuery('.web.stripped').find('.button-view').length > 0 || jQuery('.web').find('.button-view.button-view-login').length > 0 || jQuery('.web').find('.button-view.continue-btn').length > 0){
        initStrippedLoadingSpinner();
    }
});

/*
 * file: common/friends-box.js
 * author: Anders Adlén
 *
 * Adding functionality for the friend box in the header
 */
	...
	$(document).ready(function(){
		...
		$('body').mouseup(function(){
			if (document.documentElement.clientWidth < 1500) {
				if(! mouse_is_inside){
					$('.friend-box .content').slideUp({duration:100,easing:'easeInCirc'});
					$('.friend-box .tab').removeClass('open');
				}
			}
		});
		...
		$(window).scroll(function(){
			...
			if (scrollValue) {
				$('.friend-box').addClass('scrolled');
			} else {
				$('.friend-box').removeClass('scrolled');
			}
			
		});

			$('.friend-box .tab').click(function(e){
				
				if (document.documentElement.clientWidth < 1500) {
					e.preventDefault();
					if($(this).hasClass('open')){
						$('.friend-box .content').slideUp({duration:200,easing:'easeInCirc'});
						$('.friend-box .tab').removeClass('open');
					}else{
						$('.friend-box .content').slideDown({duration:200,easing:'easeOutCirc'});
						$('.friend-box .tab').addClass('open');
					}
				}
			});

		// Animate when having new friend requests
		if($('.header-notifier').hasClass('with-pending')){
			setInterval(function(){
			$('.header-notifier.with-pending span').animate({'margin-top':'-26px'},500).delay(5000).animate({'margin-top':'0px'},500).delay(5000);
		},5000);
		}
...
(function($) {
	...
	$.fn.lightBox = function(settings) {
		...
		settings = jQuery.extend({
			...
			imageLoading:			'/static/images/frontpage/lightbox-ico-loading.gif',		// (string) Path and the name of the loading icon
			imageBtnPrev:			'/static/images/frontpage/lightbox-btn-prev.gif',			// (string) Path and the name of the prev button image
			imageBtnNext:			'/static/images/frontpage/lightbox-btn-next.gif',			// (string) Path and the name of the next button image
			imageBtnClose:			'/static/images/frontpage/lightbox-btn-close.gif',		// (string) Path and the name of the close btn
			imageBlank:				'/static/images/frontpage/lightbox-blank.gif',			// (string) Path and the name of a blank image (one pixel)
...
(function (root) {
	var	$ = root.$,
		hosts = {
			'https:': 'https://www.play4free.com',
			'http:': 'http://cdn2.play4free.com'
		},
		host = hosts[root.location.protocol];
	
	// prepare Common Nav init parameters
	root.Play4Free = {
		CommonNav: {
			getInitParameterMap: function () {
				return {
					// CSS file href
					navCssLinkHref: host + '/static/theme/p4f-common-theme/css/common-nav.css',
					// flag ga from request parameter [true|false]
					gaEnabled: true,
					// ga account id
					gaAcctId: 'UA-21921048-2',
					// ga source
					utm_source: 'play4free.com',
					// ga medium
					utm_medium: 'referral',
					// ga campaign
					utm_campaign: 'p4f-common-nav'
				};
			}
		}
	};
	...
	$(root.window).one('load', function () {
		$('#p4fnav-topnav-full').show();
	});
}(this));
...
(function (root) {
	...
	if (typeof require !== 'undefined') {
		if (!root.EASY) {
			root.EASY = require('./../easy/easy');
		}
		if (!root.EASY.Session) {
			root.EASY.Session = require('./../easy/easy.session');
		}
	}
	...
	var EASY = root.EASY,
		...
		/**
		 * The common EASY application instance that used on all pages
		 * 
		 * @class APP
		 * @static
		 * @extends EASY
		 */
		APP = root.APP = new EASY();
	
	
	/**
	 * An instance of EASY.Session to store stuff in 
	 * 
	 * $property session
	 * @type {EASY.Session}
	 */
	APP.session = new EASY.Session();
...
(function (root) {
	...
	var APP = root.APP,
		_ = APP._,
		testEl = root.document && root.document.createElement('div'),
		/**
		 * @example
		 *	
		 *	{
		 *	   "CSS prefix": "events prefix"
		 *	}  
		 *
		 *  
		 * @static
	...
	/**
	 * Provides a place to put feature checks and tests
	 * 
	 * @namespace APP
	 * @class env
	 * @static
	...
	APP.env = {
		...
		canTransition: function () {
			return !!CAN_DO.transition;
		},

		canTransform: function () {
			return !!CAN_DO.transform;
		},
		
		/**
		 * @property vendorStylePrefix
		 * @type String
		 * @default ''
		 */
		vendorStylePrefix: '',
		...
		/**
		 * Prepends vendor prefix to a given css property
		 * @method prefixStyle
		 */
		prefixStyle: function (property) {
			return this.vendorStylePrefix + (property.substr(0, 1).toUpperCase() + property.substr(1));
		},
		
		/**
		 * Prepends vendor prefix to a given js event name
		 * @method prefixEvent
		 */
		prefixEvent: function (event) {
			return this.vendorEventPrefix ? this.vendorEventPrefix + event : event.toLowerCase();
		}
	};

	if (testEl) {
		_.forEach(VENDORS, function (event, vendor) {
			
			// Transform
			if (testEl.style[vendor + 'Transform'] !== undefined) {
				CAN_DO.transform = true;
				APP.env.vendorStylePrefix = vendor;
				APP.env.vendorEventPrefix = event;
			}
			// Transition
			if (testEl.style[vendor + 'Transition'] !== undefined) {
				CAN_DO.transition = true;
			}
		});
...
(function (root) {
	...
	if (typeof require !== 'undefined') {
		...
		if (!root.APP) {
			root.APP = require('./../../common/js/components/app/app');
		}
	}
	 ...
	 * @namespace nsUser
	 */
	nsUser.WalletModel = EASY.Model.extend({
		/**
		 * @property normalizeKeys
		 * @type {Object}
		 * @default {
		 *	'_ac'	: 'credits',
		 *	'_db'	: 'funds',
		...
		/**
		 * See: [http://backbonejs.org/#Model-defaults](http://backbonejs.org/#Model-defaults)
		 *
		 * @property defaults
		 * @type {Object}
		 * @default {
		...
		/**
		 * See: [http://backbonejs.org/#Model-url](http://backbonejs.org/#Model-url)
		 *
		 * @method url
		 * @return {String}
		 */
		url: function () {
			// TODO: We should use another url that this
			return APP.langUrl('/en/game/getWalletBalance');
		},
		
		
		/**
		 * Parses the request response and returns the good stuff
		 *
		 * See: [http://backbonejs.org/#Model-parse](http://backbonejs.org/#Model-parse)
		 *
		 * @method parse
		 * @param res
		...
		/**
		 * See: [http://backbonejs.org/#Sync](http://backbonejs.org/#Sync)
		 *
		 * @method sync
		 * @param method
		 * @param {String} method
		 * @param {nsUser.WalletModel} model
		 * @param {Object} [options]
		...
		sync: function (method, model, options) {
			...
			return EASY.Backbone.sync.call(this, method, model, options);
		}
	});
	 ...
	 */
	if (typeof module !== 'undefined' && module.exports) {
		module.exports = nsUser.WalletModel;
	}
}(this));
/*jshint smarttabs:true white:true */
/**
...
(function (root) {
	 ...
	 * @namespace nsUser
	 */
	nsUser.SoldierModel = EASY.Model.extend({
		/**
		 * See: [http://backbonejs.org/#Model-defaults](http://backbonejs.org/#Model-defaults)
		 * 
		 * @property defaults
	 ...
	 */
	if (typeof module !== 'undefined' && module.exports) {
		module.exports = nsUser.SoldierModel;
	}
}(this));
/*jshint smarttabs:true white:true  */
/**
...
(function (root) {
	 ...
	 * @namespace nsUser
	 */
	nsUser.UserModel = EASY.Model.extend({
		/**
		 * See: [http://backbonejs.org/#Model-defaults](http://backbonejs.org/#Model-defaults)
		 *
		 * @property defaults
		 ...
		 */
		url: function () {
			return APP.langUrl('/profile/userInfo/' + this.id);
		},
		...
		requestFriendship: function (fn) {
			...
			$.ajax({
				type: 'post',
				url: APP.langUrl('/friends/request/' + this.id),
				data: data,
				...
				error: function () {
					fn(new Error('Unable to request friendship'));
				}
			});
		...
		/**
		 * Confirm a friend request sent by this user
		 *
		 * @method confirmFriendship
		 * @param fn
		 * @chainable
		 */
		confirmFriendship: function (fn) {
			fn = fn || noop;
			...
			$.ajax({
				type: 'post',
				url: APP.langUrl('/friends/confirm/' + this.id),
				data: data,
				success: _.bind(function () {
					this.set({
						isFriend	: true,
				...
				error: function () {
					fn(new Error('Unable to confirm friendship'));
				}
			});
		...
		/**
		 * Removes this user as friend
		 *
		 * @method endFriendship
		 * @param fn
		 * @chainable
		 */
		endFriendship: function (fn) {
			fn = fn || noop;
			...
			$.ajax({
				type: 'post',
				url: APP.langUrl('/friends/remove/' + this.id),
				data: data,
				success: _.bind(function () {
					this.set({
						isFriend	: false,
				...
				error: function (xhr) {
					fn(new Error('Unable to end friendship'));
				}
			});
	 ...
	 */
	if (typeof module !== 'undefined' && module.exports) {
		module.exports = nsUser.UserModel;
	}
}(this));
/*jshint smarttabs:true white:true*/
/**
...
(function (root) {
	 ...
	 * @namespace nsUser
	 */
	nsUser.SoldiersCollection = EASY.Collection.extend({
		
		
		/**
		 * @property model
		 * @type {nsUser.SoldierModel}
		 */
		model: nsUser.SoldierModel,
		...
		initialize: function () {
			 ...
			 * @type {Number}
			 */
			this.userId = this.options.user.id;
		},
		
		
		/**
		 * Main solider should be at pos 0
		 * 
		 * See: [http://backbonejs.org/#Collection-comparator](http://backbonejs.org/#Collection-comparator)
		 * 
		 * @method comparator
		 * @param {nsUser.SoldierModel} model
		 * @returns The value that this model will be sotrted on
		 */
		comparator: function (model) {
			return !model.get('isMain');
		},
		
		
		/**
		 * Returns a localized url string
		 * 
		 * See: [http://backbonejs.org/#Collection-url](http://backbonejs.org/#Collection-url)
		 * 
		 * @method url
		 * @returns {String}
		 */
		url: function () {
			return APP.langUrl('/profile/soldiers/' + this.userId);
		},
		 ...
		 * Parses the request response and returns the good stuff
		 * 
		 * See: [http://backbonejs.org/#Collection-parse](http://backbonejs.org/#Collection-parse)
		 * 
		 * @method parse
		 * @param res
		 * @returns {Array} When successful
		 */
		parse: function (res) {
			return res && res.data;
		},
		...
		/**
		 * Finds the main solider (which should be at pos 0, but who knows)
		 * 
		 * @method getMain
		 * @return {nsUser.SoldierModel} If successful
		 */
		getMain: function () {
			return this.find(function (model) {
				return model.get('isMain');
	 ...
	 */
	if (typeof module !== 'undefined' && module.exports) {
		module.exports = nsUser.SoldiersCollection;
	}
}(this));
/*jshint smarttabs:true white:true  */
/**
...
(function (root) {
	...
	APP.ready(function () {
		...
		// On wallet change
		if (!nsUser.wallet) {
			// TODO: This is dangerously global
			var $currencyWrapper = $('div.currency-wrapper');
			
			nsUser.wallet = new nsUser.WalletModel({
				funds	: $currencyWrapper.find('span.currency_pf').text(),
				credits	: $currencyWrapper.find('span.currency_ac').text()
			});
			
			
			nsUser.wallet.bind('change', function () {
				// TODO: This is dangerously global
				var $currencyWrapper = $('div.currency-wrapper');
				
				// Set funds balance
				$currencyWrapper.find('span.currency_pf').html(nsUser.wallet.get('funds'));
				
				// Set credits balance
				$currencyWrapper.find('span.currency_ac').html(nsUser.wallet.get('credits'));
			});
		}
	});
}(this));
...
(function (root) {
	 ...
	 * @namespace nsDraw
	 */
	nsDraw.DrawModel = EASY.Model.extend({
		
		
		/**
		 * The models default attributes
		 * 
		 * @protected
		 * @property defaults
		 * @type {Object}
		...
		/**
		 * The attribute name to type mapping
		 *
		 * @protected
		 * @property schema
		 * @type {Object}
		...
		schema: {
			...
			offer: function (value) {
				...
				if (offer.currency === '_ac' || offer.currency === 'credits') {
					offer.currency = 'credits';
				} else if (offer.currency === '_db' || offer.currency === '_pf' || offer.currency === 'funds') {
					offer.currency = 'funds';
				}
				return offer;
			},
		...
		/**
		 * Returns a locale prefixed url endpoint
		 * 
		 * @method url
		 * @return {String}
		 */
		url: function () {
			return APP.langUrl('/draw/initializeDrawData');
		},
		
		
		/**
		 * Parses request responses and gets the relevant data 
		 * 
		 * @method parse
		 * @param {Object} resp
		 * @return {Object}
		...
		parse: function (resp) {
			return {
				drawBalance: resp.data.drawBalance,
				offer: {
					price: resp.data.drawOfferPrice,
					currency: resp.data.drawOfferCurrency
				},
				date: resp.data.timeToNextDraw,
				_csrf_token: resp.data._csrf_token
			};
		},
		
		
		/**
		 * Handles getting, posting, putting & deleting
		 *
		 *
		 * @method sync
		 * @param {String} method
		...
		sync: function (method, model, options) {
			...
			return EASY.Backbone.sync.call(this, method, this, options);
		},
		...
		getTimeRemaining: function () {
			return this.get('date').getTime() - new Date().getTime();
		},
		
		
		
		/**
		 * Has the time passed, should the draw be a free draw
		 * 
		 * @method hasPassed
		 * @return {Boolean}
		 */
		...
		/**
		 * Check if there is free draw available
		 * or if enough time has passed.
		 *  
		 * @return {Boolean}
		 */
		hasFreeDraw: function () {
			var drawBalance = this.get('drawBalance');
			...
			if (this.hasPassed()) {
				this.set('drawBalance', drawBalance + 1);
				return true;
			}
	 ...
	 */
	if (typeof module !== 'undefined' && module.exports) {
		module.exports = nsDraw.DrawModel;
	}
}(this));
/*jshint smarttabs:true white:true*/
/**
...
(function (root) {
	 ...
	 * @namespace nsDraw
	 */
	nsDraw.CardModel = EASY.Model.extend({
		...
		initialize: function () {
			...
			// remove category on boosters and gadgets
			if (type === 'boosters' || type === 'gadgets') {
				this.set({'categoryName': ''});
			}
		},
		
		/**
		 * Sets the image attribute, since some attachments type don't 
		 * have unique images we need to set them to a generic one for 
		 * that type.
		 * 
		 * @private
		 * @method _setImage
		 */
		_setImage: function () {
			var	image = this.get('iconUrl'),
				category = this.get('category');
			
			if (category === 'barrel' || category === 'ammo' || category === 'stock') {
				image = image.split('/');
				image[image.length - 1] = category + '.png';
				image = image.join('/');
			}
	 ...
	 */
	if (typeof module !== 'undefined' && module.exports) {
		module.exports = nsDraw.CardModel;
	}
}(this));
/*jshint smarttabs:true white:true*/
/**
...
(function (root) {
	...
	if (typeof require !== 'undefined') {
		...
		if (!root.APP.ns('draw').CardModel) {
			root.APP.ns('draw').CardModel = require('./model.card.js');
		}
	}
	 ...
	 * @extend EASY.Collection
	 * @namespace nsDraw
	 */
	nsDraw.CardsCollection = EASY.Collection.extend({
		
		
		/**
		 * The model type of the collection
		 * @property model
		 * @type {Backbone.Collection} 
		 */
		model: nsDraw.CardModel,
		
		
		/**
		 * Parses the response when fetching cards 
		 * 
		 * @method parse
		 ...
		 */
		parse: function (data) {
			if (data && data.hasOwnProperty('data') && data.data.hasOwnProperty('hand')) {
				
				return data.data.hand;
			}
		...
		/**
		 * Resets the collection to hold 8 blank cards 
		 * 
		 * @method resetToBlank
		 * @chainable
		 */
	 ...
	 */
	if (typeof module !== 'undefined' && module.exports) {
		module.exports = nsDraw.CardsCollection;
	}
}(this));
/*jshint smarttabs:true white:true */
/**
...
(function (root) {
	...
	var EASY = root.EASY,
		APP = root.APP,
		win = APP.win,
		nsDailyDraw = APP.ns('draw'),
		nsWallet = APP.ns('wallet'),
		_ = APP._,
		$ = APP.$,
		animTime = ($.browser.msie && $.browser.version < 9) ? 0 : 300;
	 ...
	 * @namespace nsDraw
	 */
	nsDailyDraw.DailyDrawView = EASY.View.extend({
		
		/**
		 * @property model
		 * @type {nsDraw.DrawModel}
		 */
		
		
		/**
		 * @property collection
		 * @type {nsDraw.CardsCollection}
		 */
		...
		/**
		 * @property soldiers
		 * @type {nsUser.SoldiersCollection}
		 */
		
		
		/**
		 * The name of the view required for all views
		 * 
		 * @static
		 * @property name
		 * @type {String}
		...
		/**
		 * Left top position of the deck of cards
		 *
		 * @property deckPos
		 * @type {Object}
		 */
		...
		/**
		 * All the DOM events that view cares about
		 *
		 * @property events
		 * @type {Object}
		 */
		events			: {
			'click div.soldier'			: '_onClickSoldier',
			'click a.switch-soldier'	: '_onClickSwitchSoldier',
			'click a.open-info'			: '_onClickOpenInfo',
			'click a.continue'			: '_onClickContinue',
			'click a.visit-faq'			: '_onClickVisitFaq',
			'click a.close-popup'		: '_onClickClosePopup',
			'click a.close-draw'		: '_onClickClose',
			'click div.container'		: '_onClickCard',
			'click a.deal-again'		: '_onClickDeal',
			'click a.reload-app'		: '_onClickReloadApp',
			'click a.juice-up'			: '_onClickJuiceUp'
		},
		...
		initialize: function () {
			...
			// Reset the collection to have 8 entries
			this.collection.resetToBlank();
			
			this.wallet = this.options.wallet;
			this.soldiers = this.options.soldiers;
			if (this.soldiers.length === 1) {
				this.currentSoldier = this.soldiers.at(0);
			}
		...
		/**
		 * When clicking on a solider we need to re-render 
		 * the view that soldiers context   
		 * 
		 * @private
		 * @method _onClickSoldier
		 * @param {jQuery.Event} e
		 */
		_onClickSoldier: function (e) {
			e.preventDefault();
			var el = e.currentTarget,
				i = this.$('div.soldier').index(el);
			
			if (i !== -1) {
				this.currentSoldier = this.soldiers.at(i);
				this.state = 'pickcard';
				this.$roster.hide();
				this._shuffle(_.bind(function () {
					this._hasDealt = false;
					this.render();
				}, this));
			}
		},
		...
		/**
		 * When click the "Switch Soldier" button show the solider roster
		 *
		 * @private
		 * @method _onClickSwitchSoldier
		 * @param {jQuery.Event} e
		 */
		_onClickSwitchSoldier: function (e) {
			e.preventDefault();
			
			this.$roster.show();
			
			this.trigger('popup:roster', this);
		},
		
		
		/**
		 * When clicking the "Continue" button prevent the 
		 * default action of that button
		 *
		 * @private
		 * $method _onClickSwitchSoldier
		 * @param {jQuery.Event} e
		 ...
		 * @param {jQuery.Event} e
		 */
		_onClickDeal: function (e) {
			e.preventDefault();
			
			// Fade out the deal button
			this.$deal.removeClass('visible');
				
			// Set state to pick card
			this.state = 'pickcard';
			
			// Shuffle
			this._shuffle(_.bind(function () {
				// Set to false so that we don't shuffle again
				this._hasDealt = false;
		...
		_onClickCard: function (e) {
			...
			var i = this.$cards.index(e.currentTarget);
		 ...
		 *
		 * @private
		 * @method _onClickOpenInfo
		 * @param {jQuery.Event} e
		 */
		_onClickOpenInfo: function (e) {
			e.preventDefault();
			
			this.$('div.dailydraw-faq').show();
			
			this.trigger('popup:faq', this);
		},
		 ...
		 * 
		 * @private
		 * @method _onClickClosePopup
		 * @param {jQuery.Event} e
		 */
		_onClickClosePopup: function (e) {
			e.preventDefault();
			
			this.$('div.popup').filter(':visible').each(_.bind(function (i, el) {
				var type = (el.className.match(/\-([^\s]+)/) || []).pop();
				$(el).hide();
				this.trigger('popup:close', this, type);
			}, this));
		},
		 ...
		 * 
		 * @private
		 * @method _onClickVisitFaq
		 * @param {jQuery.Event} e
		 */
		_onClickVisitFaq: function (e) {
			this.trigger('visitfaq', this);
		},
		 ...
		 *
		 * @private
		 * @method _onClickReloadApp
		 * @param {jQuery.Event} e
		 */
		_onClickReloadApp: function (e) {
			e.preventDefault();
		 ...
		 *
		 * @private
		 * @method _onClickJuiceUp
		 * @param {jQuery.Event} e
		 */
		_onClickJuiceUp: function (e) {
			e.preventDefault();
			
			this.$('div.popup').hide();
			
			this.$('div.dailydraw-awaiting-purchase.popup').show();
			
			this.trigger('juice-up', this);
		},
		 ...
		 * @param {Error} err
		 */
		_onError: function (view, err) {
			if (this.$cards) {
				this.$cards.removeClass('loading');
			}
			
			this.$('div.dailydraw-error.popup').show();
			
			this.trigger('popup:error', this);
		 ...
		 */
		_onRender: function () {
			this.$cards = this.$('div.container');
			this.$roster = this.$('div.dailydraw-roster.popup');
			this.$switchsoldier = this.$('a.button.switch-soldier');
			this.$overlay = this.$('div.overlay');
			this.$continue = this.$('div.container.showing div.continue');
			this.$deal = this.$('a.deal-again');
			
			// Stop countdown
			this._stopCountdown();
			
			// Show current soldier
			if (!this.currentSoldier) {
				this.$roster.show();
				this.trigger('popup:roster', this);
				this.$switchsoldier.hide();
			} else if (this.soldiers === 1) {
				this.$switchsoldier.hide();
			}
			
			// Deal cards
			if (this.state === 'pickcard') {
				
				this._rotation = _.map(_.range(8), function () {
				...
				this.$switchsoldier.hide();
				this.$overlay.show();
				this.$continue.addClass('visible');
				this.$cards.show();
				
				_.defer(_.bind(function () {
					this.$cards.filter('.showing').addClass('flipped');
					
					// Click anywhere to continue
					_.delay(_.bind(function () {
						this.$el.one('click', _.bind(function (e) {
							e.preventDefault();
							e.stopImmediatePropagation();
							this.$overlay.fadeOut(animTime);
							this.$continue.hide();
							this.$cards.removeClass('showing').addClass('flipped');
							this.$deal.delay(500).addClass('visible');
							this.$switchsoldier.show();
						}, this));
					}, this), 500);
				}, this));
			}
			
			this.$cards.each(_.bind(function (i, card) {
				var rotation = this._rotation[i];
		...
		_startCountdown: function () {
			var elems = [
				...
				getRemaining = _.bind(function () {
					...
					var timeRemaining = this.model.getTimeRemaining() / 1000,
		 ...
		 * @param {Function} fn
		 */
		_shuffle: function (fn) {
			if (this._hasDealt) {
				this.$cards.show();
				this.$cards.removeClass('flipped selected showing').animate({
					top:	this.deckPos.top,
					left:	this.deckPos.left
				}, 300, 'easeInCubic', _.after(this.$cards.size(), fn));
		...
		_deal: function () {
			...
			var positions = [-15, 153, 323, 493];
			
			this._shuffle(_.bind(function () {
				this.$cards.each(function (i) {
					var $card = $(this),
						posLeft = positions[i % 4],
						posTop = i > 3 ? 203 : -15;
						
					$card.delay(i * 200).animate({
						top:	posTop,
						left:	posLeft
					}, 100, 'easeOutCubic');
		...
		pickCard: function (card) {
			...
			if (!_.isNumber(card)) {
				i = this.collection.indexOf(card);
			}
			
			if (i !== -1 && this.state === 'pickcard') {
				// Track the card
				this.trigger('pickcard', this, i);
			
				// Can the user afford this offer?
				// Won't show this if user has free draw
				offer = this.model.get('offer');
				if (this.wallet.get(offer.currency) < offer.price && !this.model.hasFreeDraw()) {
					
					$popup = this.$('div.dailydraw-insufficient.popup');
					
					$popup.removeClass('funds credits').addClass(offer.currency);
					
					$popup.show();
					
					this.trigger('popup:insufficient', this);
				
				// Go and fetch the cards
				} else {
					this.fetchCards(this.collection.at(i));
				}
			}
		},
		...
		/**
		 * TODO: This should not be one method
		 * 
		 * 
		 *
		 * @method fetchCards
		 ...
		 * @chaining
		 */
		fetchCards: function (card) {
			var i = this.collection.indexOf(card),
				$el = this.$cards.eq(i),
				posLeft = $el.css('left'),
				posTop = $el.css('top'),
				done = _.after(2, _.bind(function () {
					this.render();
					this.collection.resetToBlank();
			...
			// Start animating card
			$el.addClass('selected').animate({
				top		: 91,
				left	: 238
			...
			this.$overlay.fadeIn(animTime);
			
			// Hide Buy buttons
			this.$cards.removeClass('buy');
			...
			this.collection.fetch({
				data: {
					personaId: this.currentSoldier.id,
					card: i,
					_csrf_token: this.model.get('_csrf_token'),
					r: Math.random() * 1000
				},
				url: APP.langUrl(this.model.hasFreeDraw() ? '/draw/drawCard' : '/draw/buyAndDrawCard'),
				// On success
				success: _.bind(function (collection, resp) {
					
					var drawBalance = resp.data.drawBalance || 0,
						timeToNextDraw = resp.data.timeToNextDraw || 0,
						offer;
					
					// If this was a paid draw?
					if (!this.model.hasFreeDraw()) {
						// Take funds from the wallet
						offer = this.model.get('offer');
						this.wallet.set(offer.currency, this.wallet.get(offer.currency) - offer.price);
						
						// TODO: This shouldn't be needed
						APP.ns('user').wallet.set(offer.currency, this.wallet.get(offer.currency));
					}
					
					// Update draw model with new data
					this.model.set({
				...
				error: _.bind(function () {
					// Trigger Error
					this.trigger('error', this, new Error('Unable to draw card!'));
				}, this)
			});
		...
		context: function () {
			var context = {
					hasFreeDraw	: this.model.hasFreeDraw(),
					state		: this.state,
					soldiers	: this.soldiers.toJSON(),
					hasDealt	: !!this._hasDealt,
					wallet		: this.options.wallet.toJSON(),
					offer		: this.model.get('offer')
				};
			
			if (this.currentSoldier) {
				context.list = this.collection.toJSON();
				context.currentSoldier = this.currentSoldier.toJSON();
			} else {
				context.list = [];
				context.currentSoldier = null;
			}
			
			return EASY.View.prototype.context.call(this, context);
		}
	}, {
		/**
		 * @static
	 ...
	 */
	if (typeof module !== 'undefined' && module.exports) {
		module.exports = nsDailyDraw.DailyDrawView;
	}
}(this));
/*jshint smarttabs:true white:true*/
/**
...
(function (root) {
	...
	var APP = root.APP,
		...
		/**
		 * Responsible for destroying the draw view
		 * and clean up everything draw related
		 * 
		 * @private
		 * @method destroyDrawView
		 */
		destroyDrawView = function (e) {
			...
			if (nsDraw.view) {
				...
				// To Some tracking
				if (nsDraw.model.get('drawBalance') > 0) {
					// TODO: We need a non-gobal whay for this
					root._gaq.push(['_trackEvent', 'Daily Draw', 'Close app', 'Free draw']);
					if (!nsDraw.view.currentSoldier) {
						// TODO: We need a non-gobal whay for this
						root._gaq.push(['_trackEvent', 'Daily Draw', 'Close app', 'Pick soldier']);
					}
				}
				...
				// Remove no scoll
				$(APP.doc.body).removeClass('noscroll');
				
				// Unbind window key up
				$(APP.win).unbind('keyup', destroyDrawView);
			}
		};
	 ...
	 */
	APP.ready(function () {
		// Should we call goto the draw directly?
		if ($('#daily-draw').size() !== 0) {
			APP.navigate('daily-draw', true);
		}
	});
	 ...
	 * @namespace nsDraw
	 */
	APP.route('daily-draw', 'daily-draw', function () {
		APP.navigate('');
		
		// Destroy draw view
		destroyDrawView();
		
		$(APP.doc.body).addClass('noscroll');
		
		// Get DOM node
		var $dailydraw = $('#daily-draw'),
			...
			handleError = function (err) {
				var $error = $('<p class="error">' + err.message + '</p>'),
					timer,
					remove = function () {
						win.clearTimeout(timer);
						$dailydraw.unbind('click', remove);
						$dailydraw.fadeOut(200, function () {
							$dailydraw.remove();
						});
					};
				
				nsDraw.trigger('error', nsDraw, err);
				
				$dailydraw.html($error);
				
				// Timer
				timer = win.setTimeout(remove, 5000);
				
				// On click
				$dailydraw.bind('click', remove);
			};
		
		// No bootstrapped root element, we need to create it
		if ($dailydraw.size() === 0) {
			$dailydraw = $('<div class="dailydraw-view" id="daily-draw"></div>').appendTo(APP.doc.body);
		}
		$dailydraw.html('<img src="/static/modules/game/loading-even-newer.gif" width="20" height="20" class="loading" />');
		$dailydraw.show();
		...
		if (!nsDraw.model) {
			 ...
			 * @type {nsDraw.DrawModel}
			 */
			nsDraw.model = new nsDraw.DrawModel();
		}
		...
		APP.async.parallel([
			...
			/**
			 * Calls fetch on the nsDraw.model to 
			 * load required data.
			 * 
			 * @private
			 * @sync
			 * @method fetchDrawData
			 * @param {Function} done
			 */
			function (done) {
				nsDraw.model.fetch({
					...
					error: function () {
						done(new Error('Unable to load draw data'));
					}
				});
			},
			/**
			...
			/**
			 * Calls fetch on the nsDraw.model
			 *
			 * @private
			 * @sync
			 * @method fetchSoldiers
			 * @param {Function} done
			 */
			function (done) {
				var soldiers = APP.session.get('soldiers');
				...
				if (soldiers.length) {
					...
					soldiers.fetch({
						...
						error: function () {
							done(new Error('Unable to load soldiers'));
						}
					});
				}
			}
			...
			function (err) {
				 ...
				 * 
				 * @property view
				 * @type {nsDraw.DailyDrawView}
				 */
				nsDraw.view = new nsDraw.DailyDrawView({
					el			: $dailydraw.get(0),
					wallet		: APP.session.get('wallet'),
					soldiers	: APP.session.get('soldiers'),
					collection	: new nsDraw.CardsCollection(),
					model		: nsDraw.model
				});
				
				// On close destroy view
				nsDraw.view.bind('close', function () {
					destroyDrawView();
				});
				
				// On reload
				nsDraw.view.bind('reload', function () {
					location.reload();
				});
				
				
				/**
				 * Opens up a new window to lockbox.
				 * Polls that window to find out when it has benn closed
				 * 
				 * @private
				 * @method event#juice-up
				 */
				nsDraw.view.bind('juice-up', function () {
					var fundWindow = APP.win.open(APP.langUrl('/store/purchaseFunds'), 'name', 'height=768,width=1024,scrollbars=yes'),
						wallet = APP.session.get('wallet'),
						pollClosed = function () {
							if (fundWindow.closed) {
								wallet.fetch({
									success: function () {
										// Track it, if it's a positive change
										
										if (wallet.previous('funds') < wallet.get('funds')) {
											// TODO: We need a non-gobal whay for this
											/**
											 * @private
											 * @event ['_trackEvent', 'Daily Draw', 'Finished purchase', 'Not enough funds']
											 */
											root._gaq.push(['_trackEvent', 'Daily Draw', 'Finished purchase', 'Not enough funds']);
										} else {
											// TODO: We need a non-gobal whay for this
											/**
											 * @private
											 * @event ['_trackEvent', 'Daily Draw', 'Cancelled purchase', 'Not enough funds']
											 */
											root._gaq.push(['_trackEvent', 'Daily Draw', 'Cancelled purchase', 'Not enough funds']);
										}
										// We need to render
										nsDraw.view.render();
									}
					...
					/**
					 * @private
					 * @event ['_trackEvent', 'Daily Draw', 'Continue', 'Not enough funds']
					 */
					root._gaq.push(['_trackEvent', 'Daily Draw', 'Continue', 'Not enough funds']);
				...
				/**
				 * Funding GA tracking event
				 * 
				 * @private
				 * @method event#popup:insufficient
				 */
				nsDraw.view.bind('popup:insufficient', function () {
					// TODO: We need a non-gobal whay for this
					/**
					 * @private
					 * @event ['_trackEvent', 'Daily Draw', 'Open', 'Not enough funds']
					 */
					root._gaq.push(['_trackEvent', 'Daily Draw', 'Open', 'Not enough funds']);
				});
				 ...
				 * @method event#render
				 */
				nsDraw.view.once('render', function () {
					/**
					 * Switch soldier GA tracking event
					 * 
					 * @private
					 * @method event#popup:roster
					 */
					nsDraw.view.bind('popup:roster', function () {
						// TODO: We need a non-global way for this
						/**
						 * @private
						 * @event ['_trackEvent', 'Daily Draw', 'Open', 'Switch soldier']
						 */
						root._gaq.push(['_trackEvent', 'Daily Draw', 'Open', 'Switch soldier']);
					});
				});
				
				
				/**
				 * Deal again GA tracking event
				 * 
				 * @private
				 * @method event#dealagain
				 */
				nsDraw.view.bind('dealagain', function () {
					/**
					 * @private
					 * @event ['_trackEvent', 'Daily Draw', 'Shuffle', 'Deal Again']
					 */
					root._gaq.push(['_trackEvent', 'Daily Draw', 'Shuffle', 'Deal Again']);
				});
				
				
				/**
				 * Info GA tracking event
				 *
				 * @private
				 * @method event#popup:faq
				 */
				nsDraw.view.bind('popup:faq', function () {
					// TODO: We need a non-gobal whay for this
					/**
					 * @private
					 * @event ['_trackEvent', 'Daily Draw', 'Open', 'Info']
					 */
					root._gaq.push(['_trackEvent', 'Daily Draw', 'Open', 'Info']);
				});
				
				
				/**
				 * Visit FAQ GA tracking event
				 *
				 * @private
				 * @method event#visitfaq
				 */
				nsDraw.view.bind('visitfaq', function () {
					// TODO: We need a non-gobal whay for this
					/**
					 * @private
					 * @event ['_trackEvent', 'Daily Draw', 'Read more', 'Info']
					 */
					root._gaq.push(['_trackEvent', 'Daily Draw', 'Read more', 'Info']);
				});
				
				// Pick card GA tracking event
				nsDraw.view.bind('pickcard', function (view, i) {
					// TODO: We need a non-gobal whay for this
					/**
					 * @private
					 * @event ['_trackEvent', 'Daily Draw', 'Pick card', i + 1
					 */
					root._gaq.push(['_trackEvent', 'Daily Draw', 'Pick card', i + 1]);
				});
				...
				// On window key up
				$(APP.win).bind('keyup', destroyDrawView);
			});
	});
}(this));