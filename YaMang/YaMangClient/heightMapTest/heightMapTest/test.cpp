...
#define WINDOW_W  500//윈도 너비
#define WINDOW_H  500//윈도 높이
#define WINDOW_TITLE L"HeightMap-TList"//윈도 타이틀
#define BMP_HEIGHTMAP L"map128.bmp"//파일 이름
/**-----------------------------------------------------------------------------
*  전역변수
*------------------------------------------------------------------------------
*/
HWND     g_hwnd = NULL;//윈도핸들
...
D3DXMATRIXA16   g_matAni;//행렬
DWORD     g_cxHeight = 0;//x축
DWORD     g_czHeight = 0;//z축
//xz평면
/// 사용자 정점을 정의할 구조체
struct CUSTOMVERTEX
{
...
HRESULT InitTexture( )
{
	...
	if ( FAILED( D3DXCreateTextureFromFileEx( g_pd3dDevice, BMP_HEIGHTMAP, //장치,위에서 지정한  파일이름
...
HRESULT InitVB( )
{
	D3DSURFACE_DESC  ddsd;//지형.
	D3DLOCKED_RECT  d3drc;//영역
	g_pTexHeight->GetLevelDesc( 0, &ddsd ); /// 대상의 텍스처의 정보
	...
	if ( FAILED( g_pVB->Lock( 0, g_cxHeight*g_czHeight*sizeof( CUSTOMVERTEX ), (void**) &pVertices, 0 ) ) )
		return E_FAIL;//너비와.높이만큼의.각.버텍스를.생성하고..그.포인터를.pVertices로 리턴한다.
	CUSTOMVERTEX v;//버텍스
	CUSTOMVERTEX* pV = (CUSTOMVERTEX*) pVertices;//주소값들을 복사
	for ( DWORD z = 0; z < g_czHeight; z++ )//높이
	{
		for ( DWORD x = 0; x < g_cxHeight; x++ )//너비
		{
			v.p.x = (float) x - g_cxHeight / 2.0f;  /// 정점의 x좌표(메시를 원점에 정렬)..너비에서.너비의 절반을 뺀다.
			...
			v.n.y = v.p.y;
			v.n.z = v.p.z;//노멀
			D3DXVec3Normalize( &v.n, &v.n );//연산결과,연산입력...3d벡터의 정규화한 벡터를 돌려준다.
			v.t.x = (float) x / ( g_cxHeight - 1 );//텍스처..원점.
...
VOID Animate( )
{
	...
	//flag가 true라면 wire모드로 한다.
	g_pd3dDevice->SetRenderState( D3DRS_FILLMODE, flag ? D3DFILL_WIREFRAME : D3DFILL_SOLID );
	t = d;//d의 값은 시간이 증가함에따라 증가하고 이 값을 t에 저장한다.
	//1초가 지나면 d는 가질수 있는 값중 가장 작은 값을 가지게 되고. 이 때 t는 d의 가장 큰 값을
	//저장해 있는 상태이므로 이를 이용해 flag를 조정한다. 그 이후 다시 둘은 같은값이된다.
	LogFPS( );
}
